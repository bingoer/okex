package com.weber.okex.ticker.client.security;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean isApiKeyRequired = original.header(OkexApiConstants.ENDPOINT_SECURITY_TYPE_APIKEY) != null;
        boolean isSignatureRequired = original.header(OkexApiConstants.ENDPOINT_SECURITY_TYPE_SIGNED) != null;
        newRequestBuilder.removeHeader(OkexApiConstants.ENDPOINT_SECURITY_TYPE_APIKEY)
            .removeHeader(OkexApiConstants.ENDPOINT_SECURITY_TYPE_SIGNED);

        // Endpoint requires sending a valid API-KEY
        if (isApiKeyRequired || isSignatureRequired) {
            newRequestBuilder.addHeader(OkexApiConstants.API_KEY_HEADER, apiKey);
        }
