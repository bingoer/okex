package com.weber.okex.ticker.okexclient.rest.stock;
...
public interface IStockRestApi {
  ...
  public String trades(String symbol, String since) throws HttpException, IOException;
	 ...
	 * @param size 可选，指定获取数据的条数
	 * @param since 可选，时间戳，返回该时间戳以后的数据(例如1417536000000)
	 * @return
	 * @throws IOException
	 * @throws HttpException
	 */
   ...
   * @param price 下单价格 [限价买单(必填)： 大于等于0，小于等于1000000 | 市价买单(必填)： BTC :最少买入0.01个BTC 的金额(金额>0.01*卖一价) /
   *     LTC :最少买入0.1个LTC 的金额(金额>0.1*卖一价)]
   * @param amount 交易数量 [限价卖单（必填）：BTC 数量大于等于0.01 / LTC 数量大于等于0.1 | 市价卖单（必填）： BTC :最少卖出数量大于等于0.01 /
   *     LTC :最少卖出数量大于等于0.1]
   * @return
   * @throws IOException
   * @throws HttpException
   ...
   * @throws HttpException
   */
  public String batch_trade(String symbol, String type, String orders_data)
      throws HttpException, IOException;
   ...
   * @throws HttpException
   */
  public String orders_info(String type, String symbol, String order_id)
      throws HttpException, IOException;
