...
@Component
public class PriceStrategy3Min<T> extends AbstratStractegy<KlineResult> {
  ...
  public PriceStrategy3Min<T> build(String symbol, List<OkexKline> klines) {
    this.symbol = symbol;
    this.klines = klines;
    return this;
  }
  ...
  public KlineResult execute() {
    ...
    if (first.getClose().divide(first.getOpen(), 2, BigDecimal.ROUND_HALF_UP).compareTo(new BigDecimal("1.02")) >= 0) {
      msg += MessageFormat.format("3分钟内价格涨了2%以上[after:{0},before:{1}]-",
      ...
      if (secend.getClose().subtract(secend.getOpen()).compareTo(BigDecimal.ZERO) >= 0) {
        msg += MessageFormat.format("3分钟前价格也是上涨[after:{0},before:{1}]-",
